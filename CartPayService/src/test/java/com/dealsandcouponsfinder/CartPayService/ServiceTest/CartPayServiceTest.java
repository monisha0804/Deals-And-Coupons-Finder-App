...
public class CartPayServiceTest {
	...
	public void saveTest() {
		Cart cart = new Cart("1111111000","monisha@gmail.com",5,"1234",20);
	...
	public void findByCartIdTest() throws CartPayRequestException {
		when (cartRepository.findById("2")).thenReturn(Optional.of(new Cart("2","monisha@gmail.com",5,"1234",20)));
		Optional<Cart> cart = cartService.findByCartId("2");
		assertEquals("2", cart.get().getCartId());
		assertEquals("monisha@gmail.com",cart.get().getUserId());
	...
	public void findByUserIdTest() throws CartPayRequestException {
		when (cartRepository.findById("monisha@gmail.com")).thenReturn(Optional.of(new Cart("2","monisha@gmail.com",5,"1234",20)));
		Optional<Cart> cart = cartService.findByCartId("monisha@gmail.com");
		assertEquals("2", cart.get().getCartId());
		assertEquals("monisha@gmail.com",cart.get().getUserId());
		assertEquals(5,cart.get().getQuantity());
		assertEquals("1234",cart.get().getCouponId());
